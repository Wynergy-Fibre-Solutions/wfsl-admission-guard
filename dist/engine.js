...
function isExpired(surface, now) {
    if (!surface.expires_at)
        return false;
...
function buildEvidence(manifest, input, outcome, code, status, now) {
    return {
...
export function parseManifestJson(jsonText) {
    try {
...
export function decideAdmission(manifest, input) {
    ...
    if (!ok) {
        const evidence = buildEvidence(undefined, { pathname, method, now }, "ERROR", "ADMISSION_MANIFEST_INVALID", 500, now);
        return {
            ok: false,
            outcome: "ERROR",
            status: 500,
    ...
    if (!surface) {
        ...
        const evidence = buildEvidence(typed, { pathname, method, now }, "REFUSED", "SURFACE_NOT_ADMITTED", status, now);
        return { ok: true, outcome: "REFUSED", status, evidence };
    }
    if (isExpired(surface, now)) {
        const status = typed.defaults?.refusal_status ?? 451;
        const evidence = buildEvidence(typed, { pathname, method, now }, "REFUSED", "SURFACE_EXPIRED", status, now);
        return { ok: true, outcome: "REFUSED", status, evidence };
    }
    if (surface.kind === "api") {
        ...
        if (!methods.includes(method)) {
            const status = typed.defaults?.refusal_status ?? 451;
            const evidence = buildEvidence(typed, { pathname, method, now }, "REFUSED", "METHOD_NOT_ADMITTED", status, now);
            return { ok: true, outcome: "REFUSED", status, evidence };
        }
    }
    const evidence = buildEvidence(typed, { pathname, method, now }, "ADMITTED", "ADMITTED", 200, now);
